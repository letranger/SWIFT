...
#+latex:\newpage
#+begin_export html
<a href="https://hits.sh/letranger.github.io/Swift/Firebase.html/"><img align="right" alt="Hits" src="https://hits.sh/letranger.github.io/Swift/Firebase.html.svg"/></a>
#+end_export
...
a. Cloud Firestore: is a flexible, scalable database for mobile, web, and server development from Firebase and Google Cloud Platform. It is a *NoSQL document database* that lets you easily *store, sync, and query data* for your mobile and web apps — at a global scale.It's supporting for Android, iOS and Web Platform.
b. ML Kit: is a mobile SDK that brings *Google’s machine learning expertise to Android and iOS apps* in a powerful yet easy-to-use package. Whether you’re new or experienced in machine learning, you can implement the functionality you need in just a few lines of code. There’s no need to have deep knowledge of neural networks or model optimization to get started. On the other hand, if you are an experienced ML developer, ML Kit provides convenient APIs that help you use your custom TensorFlow Lite models in your mobile apps. It’s supporting for Android and iOS Platform.
c. Cloud Functions: for Firebase lets you automatically run backend code in response to events *triggered* by Firebase features and HTTPS requests. Your code is stored in Google’s cloud and runs in a managed environment. There’s no need to manage and scale your own servers. It’s supporting for Android, iOS, C++, Unity and Web Platform.
d. Authentication: provides backend services, easy-to-use SDKs, and ready-made UI libraries to *authenticate users to your app*. It supports authentication using passwords, phone numbers, popular federated identity providers like Google, Facebook and Twitter, and more. It’s supporting for Android, iOS and Web Platform.
...
f. Cloud Storages: is for *object storage service* built for Google scale. The Firebase SDKs for Cloud Storage add Google security to file uploads and downloads for your Firebase apps, regardless of network quality. You can use our SDKs to store images, audio, video, or other user-generated content. On the server, you can use Google Cloud Storage, to access the same files. It’s supporting for Android, iOS, C++, Unity and Web Platform.
g. Real-time Database: is a cloud-hosted NoSQL database that lets you *store and sync between your users in real-time*. The Real-time Database is really just *one big JSON object* that the developers can manage in real-time. It’s supporting for *Android, iOS, C++, Unity and Web Platform*.
...
5. Create a Cloud Firestore database
   a) If you haven't already, create a Firebase project: In the Firebase console, click *Add project*, then follow the on-screen instructions to create a Firebase project or to add Firebase services to an existing GCP project.
   b) Navigate to the *Cloud Firestore section* of the *Firebase console*. You'll be prompted to select an existing Firebase project. Follow the database creation workflow.
   c) Select a starting mode for your Cloud Firestore Security Rules:
      - Locked mode
...
service cloud.firestore {
  match /databases/{database}/documents {
   ...
   d) Select a location for your database.
...
1) If you've previously used CocoaPods, remove them from the project with pod deintegrate.
2) Install Firebase via Swift Package Manager:
   #+CAPTION: Install Firebase SDK #1
   #+LABEL:fig:sdk-1
   #+name: fig:sdk-1
   #+ATTR_LATEX: :width 300
   #+ATTR_ORG: :width 300
   #+ATTR_HTML: :width 500
   [[file:images/sdk-1.jpg]]
3) Select the Firebase GitHub repository
   ios sdk URL: https://github.com/firebase/firebase-ios-sdk.git:
   #+CAPTION: Install Firebase SDK #2
   #+LABEL:fig:sdk-2
   #+name: fig:sdk-2
   #+name: fig:FigName
   #+ATTR_LATEX: :width 300
   #+ATTR_ORG: :width 300
   #+ATTR_HTML: :width 500
   [[file:images/sdk-2.jpg]]
4) Select the Branch: maser
   #+CAPTION: Install Firebase SDK #3
   #+LABEL:fig:sdk-3
   #+name: fig:sdk-3
   #+name: fig:FigName
   #+ATTR_LATEX: :width 300
   #+ATTR_ORG: :width 300
   #+ATTR_HTML: :width 500
   [[file:images/sdk-3.jpg]]
5) Wait about 2000 years
   #+CAPTION: Install Firebase SDK #3
   #+LABEL:fig:sdk-3
   #+name: fig:sdk-3
   #+name: fig:FigName
   ...
   #+ATTR_HTML: :width 500
   [[file:images/sdk-3.jpg]]
6) Choose the Firebase products that you want installed in your app.
   #+CAPTION: Install Firebase SDK #4
   #+LABEL:fig:sdk-4
   #+name: fig:sdk-4
   #+name: fig:FigName
   #+ATTR_LATEX: :width 300
   #+ATTR_ORG: :width 300
   #+ATTR_HTML: :width 500
   [[file:images/sdk-4.jpg]]
7) DEMO
   #+CAPTION: Install Firebase SDK #DEMO
   #+LABEL:fig:sdk-DEOM
   #+name: fig:sdk-DEOM
   #+name: fig:FigName
   #+ATTR_LATEX: :width 300
   ...
   [[file:images/sdk.gif]]
**** Initialize Project
1. Open your project (xcworkspace)
2. add config file (GoogleService-info.plist) to your project
...
struct ContentView: View {
    var body: some View {
        let store = Firestore.firestore()
        // 這個Firebase資料庫物件為日後存取、增刪Firebase資料
        Button(action: {
    ...
    func loadData() {
        db.collection("tasks").addSnapshotListener { (querySanpshot, error) in
            ...
            if let querySnapshot = querySanpshot {
                // 把Firebase傳回來的documents轉為程式所需的Model
...
* Add, Delete #1 data from Cloud: UI with Firebase [MVVM]
:PROPERTIES:
:CUSTOM_ID: SW-Delete
:END:
** Repository
...
func add(task: Task) {
        do {
            _ = try db.collection("todo").addDocument(from: task)
        } catch {
            fatalError("Add a task failed")
        }
    }

    func delete(indexset: IndexSet) {
        // Just show what does indexset contain
        for i in indexset {
            print(i)
        }
        indexset.forEach { index in
            let task = tasks[index]
            db.collection("todo").document(task.id!).delete() { error in
                if let error = error {
                    fatalError("Deleting a task failed, \(error.localizedDescription)")
                }
...
//  FireTask
//
//  Created by yen yung chin on 2021/3/31.
//
        ...
        NavigationView {
            VStack {
                ...
                Button(action: {
                    ...
                    taskListVM.add(task: task)
                }, label: {
                    Text("Add Task")
                }).disabled(title == "") // trick 
                Form {
                    ForEach(taskListVM.taskCellViewModels) { taskCellVM in
                        Text("\(taskCellVM.task.title)")
                    }
                    .onDelete(perform: { indexSet in
                        taskListVM.delete(indexset: indexSet)
                    })
                }
                .navigationBarTitle(Text("Task Manager"), displayMode: .inline)
            }
        }.padding()
    }
}
...
//  FireTask
//
//  Created by yen yung chin on 2021/4/1.
//
import Foundation
import Combine
...
class TaskListViewModel: ObservableObject {
    ...
    init() {
        taskRepository.loadData()
        // 讀取Firebase docs後，轉換id，以taskCellViewModel陣列儲存
        taskRepository.$tasks.map { tasks in
            tasks.map(TaskCellViewModel.init)
        }
        .assign(to: \.taskCellViewModels, on: self)
    ...
    func delete(indexset: IndexSet) {
        // pass indexSet
        taskRepository.delete(indexset: indexset)
    }
}
#+end_src

* Delete #2 data from Cloud: UI with Firebase [MVVM]
** Repository
#+begin_src python -r -n :results output :exports both
//
//  TaskRepository.swift
...
class TaskRepository: ObservableObject {
    ...
    func loadData() {
        ...
        db.collection("todo").addSnapshotListener { (querySanpshot, error) in
    ...
    func delete(task: Task) {
        ...
        db.collection("todo").document(documentID).delete { error in
            if let error = error {
                print("Deleteing a task failed, \(error.localizedDescription)")
            }
        }
    }
}
        ...
        NavigationView {
            VStack {
                ...
                Form {
                    ...
                    .onDelete(perform: { indexSet in
                        indexSet.map {
                            taskListVM.taskCellViewModels[$0].task
                        }.forEach(taskListVM.delete(task:))
                    })
                }
                .navigationBarTitle(Text("Task Manager"), displayMode: .inline)
            }
...
* Detail/Edit data from Cloud: UI with Firebase [MVVM]
:PROPERTIES:
:CUSTOM_ID: SW-Detail
:END:
** Repository
...
class TaskRepository: ObservableObject {
    ...
    func update(task: Task) {
        guard let documentID = task.id else {return}
        do {
            try db.collection("todo").document(documentID).setData(from: task)
        } catch {
            fatalError("Update a task failed, \(error.localizedDescription)")
        }
    }
}
#+end_src
        ...
        NavigationView {
            VStack {
                ...
                Form {
                    ForEach(taskListVM.taskCellViewModels) { taskCellVM in
                        NavigationLink(
                            destination: TaskDetailView(taskCellVM: taskCellVM),
                            label: {
                                Text("\(taskCellVM.task.title)")
                            })
                    }
...
//  FireTask
//
//  Created by yen yung chin on 2021/4/3.
//
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Done")) {
                    Text(taskCellVM.task.completed ? "Completed" : "Not Yet" )
                }
            }
            .navigationBarTitle("Detail")
            .navigationBarTitleDisplayMode(.inline)
            .navigationBarItems(trailing: NavigationLink(
                                    destination: UpdateTaskView(taskCellVM: taskCellVM),
                                    label: {
                                        Text("Update")
                                    }))
        }
...
struct TaskCellView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        TaskDetailView(taskCellVM: TaskCellViewModel(task: Task(id: "", title: "", completed: false)))
    }
}
...
struct UpdateTaskView: View {
    ...
    var body: some View {
        Form {
            Section(header: Text("Updage detail")) {
                TextField("title", text: $taskCellVM.task.title)
            }
            Section(header: Text("Done")) {
                Toggle("Task Completed", isOn: $taskCellVM.task.completed)
            }
            Button(action: {
                taskCellVM.update(task: taskCellVM.task)
                self.presentationMode.wrappedValue.dismiss()
            }, label: {
                Text("Update")
            })
        }
        .navigationBarTitle("Updating")
        .navigationBarTitleDisplayMode(.inline)
    }
}
...
struct UpdateTaskView_Previews: PreviewProvider {
    static var previews: some View {
        UpdateTaskView(taskCellVM: TaskCellViewModel(task: Task(id: "", title: "", completed: false)))
    }
}
...
class TaskCellViewModel: ObservableObject, Identifiable {
    ...
    private var cancellables = Set<AnyCancellable>()
    var id = ""
    // 把Firestore document的id轉換成model struct的欄位
    init(task: Task) {
        self.task = task
        $task
            .compactMap { $0.id }
...
#+end_src
*** DEMO
#+CAPTION: Detail / Edit DEMO
#+LABEL:fig:Detail-Edit-Demo
#+name: fig:Detail-Edit-Demo
#+ATTR_LATEX: :width 300
#+ATTR_ORG: :width 300
#+ATTR_HTML: :width 300
[[file:images/Update-Edit.gif]]

** Topics:
- Detection of Modified: [[https://www.youtube.com/watch?v=imTjGvSjBEw][Adding data to Firestore from a SwiftUI App]]
...
* Storage
:PROPERTIES:
:CUSTOM_ID: SW-Firebase-Storage
:END:
** preparation
1. Add Firebase/Strage Library
2. Restart Xcode
3. Check if you can import FirebaseStorage
4. Enable Firebase/Storage on console
5. Change Storage Rules
...
service firebase.storage {
  match /b/<bucket>/o {
   ...
   #+end_src
[[https://firebase.googleblog.com/2016/07/5-tips-for-firebase-storage.html][5 tips for Firebase Storage ]]

** How to pick image in iOS
:PROPERTIES:
:CUSTOM_ID: SW-ImagePicker
:END:
*** Utility: ImagePicker
ImagePicker.swift
#+begin_src swift :noevl
import SwiftUI
...
#+end_src
*** How to call ImagePicker in SwiftUI
TaskNewView.swift
#+begin_src python -r -n :results output :exports both
//
//  TaskNewView.swift
//  FireTask
//
//  Created by yen yung chin on 2021/5/3.
//
...
struct TaskNewView: View {
    ...
    var body: some View {
        VStack {
            Image(uiImage: ((pickedImage ?? UIImage(systemName: "camera"))!))
                .resizable()
            ...
            Button("Add Task") {
                ...
                let task = Task(title: self.title, imageRefStr: "", imageUrlStr: "", completed: false)
                taskListVM.add(task: task, image: pickedImage!)
                self.presentationMode.wrappedValue.dismiss()
            }.disabled(title == "" || pickedImage==nil)
        ...
        .sheet(isPresented: $showPhotoLibrary, onDismiss: {
            ...
            ImagePicker(image: self.$pickedImage, isShown: self.$showPhotoLibrary)
        })
    }
}

struct TaskNewView_Previews: PreviewProvider {
    static var previews: some View {
        TaskNewView()
    }
}
...
** Just Upload Image to Firebase Storage
:PROPERTIES:
:CUSTOM_ID: SW-UploadImage
:END:
#+begin_src swift -r -n :results output :exports nil
// 隨機產生上傳的檔名
let imageRefStr = "images/\(UUID().uuidString).jpg"
// 設好檔案的storage reference
let imageRef = Storage.storage().reference().child(imageRefStr)
// 壓縮圖片
guard let sureImage = image.jpegData(compressionQuality: 1) else {
    print("error when processing image compression")
    return
}
// 上傳
imageRef.putData(sureImage, metadata: nil) { (meta, err) in
    if let err = err {
        print(err)
        return
    }}
...
** How to upload image to Firebase storage and get download URL (for saving into database)
:PROPERTIES:
:CUSTOM_ID: SW-UploadSaveImage
:END:
...
class TaskRepository: ObservableObject {
    ...
    func add(task: Task, image: UIImage) {
        // 設定圖片檔名及存檔位置
        let imageRefStr = "images/\(UUID().uuidString).jpg"
        // 以上述資料建立Storage reference
        let imageRef = Storage.storage().reference().child(imageRefStr)
        // 上傳 前對圖片進行壓縮
        guard let imageData = image.jpegData(compressionQuality: 0.5) else {
            print("compress jpg failed")
            return
        }
        // 上傳圖片
        imageRef.putData(imageData, metadata: nil, completion: { (metadata, error) in
            // 若出現錯誤則輸出錯誤、結束add
            if let error = error {
                print("putData Errir: \(error.localizedDescription)")
                return
            }
            // 取得該圖片的下載網址
            imageRef.downloadURL(completion: { (url, error) in
                if let error = error {
                    print("getting downloadURL Errir: \(error)")
                    return
                }
                // 確定有拿到網址
                ...
                // 成功上傳檔案、取得下載網址，進行新增任務
                let newTask = Task(title: task.title, imageRefStr: imageRefStr, imageUrlStr: urlString, completed: task.completed)
                do {
                    _ = try self.db.collection("todo").addDocument(from: newTask)
                } catch {
                    fatalError("Add a task failed")
                }
            })
    ...
    func delete(task: Task) {
        // pass in Task
        guard let documentID = task.id else { return }
        let imageRef = Storage.storage().reference().child(task.imageRefStr)

        imageRef.delete { error in
            if let error = error {
                // Uh-oh, an error occurred!
                print("Unable to delete image, error: \(error)")
            } else {
                // File deleted successfully
                // Now Delete Task

                self.db.collection("todo").document(documentID).delete { error in
                    if let error = error {
                        print("Deleteing a task failed, \(error.localizedDescription)")
                    }
                }
...
** Show online image: URLImage
:PROPERTIES:
:CUSTOM_ID: SW-URLImage
:END:
*** url-image
            ...
            Form {
                Section(header: Text("Task")) {
                    VStack (alignment: .center, spacing: 20) {
                        Text(taskCellVM.task.title)
                        Divider()
                        URLImage(URL(string: taskCellVM.task.imageUrlStr)!) { Image in
                            Image
                                .resizable()
...
struct TaskDetailView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        TaskDetailView(taskCellVM: TaskCellViewModel(task: Task(id: "", title: "", imageRefStr: "", imageUrlStr: "",  completed: false)))
    }
}
...
** Videos
- [[https://www.youtube.com/watch?v=GGp-wT693IY][Downloading Images in SwiftUI]]
- [[https://www.youtube.com/watch?v=1HN7usMROt8][Firebase Authentication Tutorial 2020 - Custom iOS Login Page (Swift)]]
...
- [[https://www.youtube.com/watch?v=ZCFz9Mj4gkU][#1 Grocery App - Setting Up Firebase with Xcode 12]]
- [[https://www.youtube.com/watch?v=dxgQZWIdRC0&t=339s][#2 Grocery App - Saving Grocery Store to Firestore Database]]
- [[https://www.youtube.com/watch?v=XXcrBVViFv4][#3 Grocery App - Fetching and Displaying Stores]]
- [[https://www.youtube.com/watch?v=4d3ctNyKNbE][#4 Grocery App - Deleting Store from Firestore Database Using SwiftUI]]
- [[https://www.youtube.com/watch?v=nYSqDY-dqT4][#5 Grocery App - Updating Store in Firestore Database Using SwiftUI]]
- [[https://www.youtube.com/watch?v=1HN7usMROt8][Firebase Authentication Tutorial 2020 - Custom iOS Login Page (Swift)]]
- [[https://www.youtube.com/watch?v=tdyT_v4lwos][Storing Custom Objects in @AppStorage in SwiftUI for iOS 14]]
