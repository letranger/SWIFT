...
class TaskRepository: ObservableObject {
    ...
    func add(task: Task, image: UIImage) {
        let imageRefStr = "images/\(UUID().uuidString).jpg"
        // Upload image and return URL and add task
        let imageRef = Storage.storage().reference().child(imageRefStr)
...
** How to show online image: Kingfisher
*** Install 3rd party swift library
        ...
        NavigationView {
            Form {
                Section(header: Text("Task")) {
                    VStack {
                        Text(taskCellVM.task.title)
                        KFImage(URL(string: taskCellVM.imageURLString))
                            .resizable()
                            .aspectRatio(contentMode: .fit)
