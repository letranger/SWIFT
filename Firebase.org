...
struct ContentView: View {
    ...
    var body: some View {
        ...
        VStack {
            TextField("Task title here...", text: self.$title)
            Toggle("Completion", isOn: self.$completion)
            Button(action: {
                store.collection("tasks").addDocument(data: ["title": self.title, "completion" : self.completion]) { err in
                    if let err = err {
                        print("Error adding document: \(err).")
                    } else {
                        print("Adding Document successfully.")
  ...
  #+end_src
- UI for getting document
  #+begin_src swift -r -n :results output :exports nil

  #+end_src
** UI with Firebase [MVVM]
