...
#+end_src
主程式 click button 後，秀出目標 View (order), 下單 View 內容為：
#+begin_src swift -r -n :results output :exports nil
import SwiftUI
...
一個 tab bar 可以包含無數個 tab，但可以能容納的數量視乎根據手機/平板的大小，以及橫、豎屏模式都會有所影響。在空間的限制下，當某些 tab 無法被顯示時，最後一個 tab 會變成“更多”(More)， 通過這個 tab 可以到另一個獨立的列表頁面，那裡會列出所有無法被顯示的 tab。[fn:10]
...
:END:
適用情境：app 中某些 property 需要於很多 View 中存取，例如，app 的 setting
** ContentView
